...
def microbial(genes,world,position):
    ...
    if fitness1>fitness2:
        ...
        genes[ind_2]=[copy.deepcopy(crossover(gene2,mutation(gene1))),fitness2]
        genes[ind_1]=[gene1,fitness1]
    elif fitness2>fitness1:
        gene1=copy.deepcopy((gene2)) #crossover
        genes[ind_1]=[copy.deepcopy(crossover(gene1,mutation(gene2))),fitness1]
        genes[ind_2]=[gene2,fitness2]
    
    return genes,max(fitness1,fitness2)

def getTopGenes(genes,x):
    #get the top x genes that performed
    arr=[]
    for key in genes:
        p=genes[key][1]
        placed=False
...
for i in range(epochs):
    ...
    topGenes=getTopGenes(gene_pop,3) #gather best genes
    #repopulate
    gene_pop={}
