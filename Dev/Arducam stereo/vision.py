...
retA, imgOriginalSceneA = capA.read()
retB, imgOriginalSceneB = capB.read()
Pastdisp,frame0_new,frame1_new=readAndStereo(imgOriginalSceneA,imgOriginalSceneB)
c=0
while not_broken: #loop through
    retA, imgOriginalSceneA = capA.read()
    retB, imgOriginalSceneB = capB.read()
    #=imgOriginalSceneA#[:len(imgOriginalSceneA)-40]
    #imgOriginalSceneB=imgOriginalSceneB[40:]
    cv.imwrite(DATAPATH+str(c)+"L.png",imgOriginalSceneA)
    cv.imwrite(DATAPATH+str(c)+"R.png",imgOriginalSceneB)
